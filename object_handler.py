...
class ObjectHandler:
    def __init__(self, game):
        ...
        # spawn npc
        self.npc_types = [SoldierNPC, CacoDemonNPC, CyberDemonNPC, ImpNPC, DemonNPC]
        if(map.map_index == 0):
            self.enemies = 20  # npc count
            self.weights = [40, 15, 0, 20, 25]
        elif (map.map_index == 1):
            self.enemies = 30  # npc count
            self.weights = [20, 15, 0, 30, 35]
        elif (map.map_index == 2):
            self.enemies = 40  # npc count
            self.weights = [10, 25, 0, 30, 35]
        elif (map.map_index == 3):
            self.enemies = 45  # npc count
            self.weights = [0, 35, 0, 30, 35]
        elif (map.map_index == 4):
            self.enemies = 1  # npc count
            self.weights = [0, 0, 100, 0, 0]
